// NOTE: alternative of the armadillo shader for cyan tinting
...
void main() {
    ...
    vec3 albedo = uAlbedo;
    if (uUseAlbedoTex) {
        albedo = texture(uAlbedoTex, TexCoords).rgb;
    }

    float tintThreshold = 40.0;
    // orb-based tint (cyan inside orb)
    float d = distance(FragPos, orbPosition);
    if (d < max(min(orbRadius, 45.0) - tintThreshold, 0.0)) {
        vec3 tint = vec3(0.0, 1.0, 1.0);
        albedo = tint; // just make it full tint lol //albedo *= tint; 
    }

    float ao = uAO;
    if (uUseAOTex) {
        ao = texture(uAOTex, TexCoords).r;
    }
    ao = clamp(ao, 0.0, 1.0);

    // also note the armadillo's normals are inverted
    ...
    if (uUseRoughTex) {
        roughness = texture(uRoughTex, TexCoords).r;
    }
    roughness = clamp(roughness, 0.0, 1.0);

    float metallic = uMetallic;
    if (uUseMetalTex) {
        metallic = texture(uMetalTex, TexCoords).r;
    }
